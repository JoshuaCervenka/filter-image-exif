...
void send_clicked(GtkWidget *widget, gpointer data){
  system("./sendJpeg_SF.bash");  //runs the send jpeg app
...
int main (int argc, char *argv[])
{
    ...
    gtk_window_set_title (GTK_WINDOW (window), "Send JPEG");
    ...
    send_button = gtk_button_new_with_label("Send JPEG");
    //gtk_fixed_put(GTK_FIXED(fixed), send_button, 200, 450);
    gtk_widget_set_size_request(send_button, 140, 25);
    ...
    gtk_container_add(GTK_CONTAINER(button_hbox), send_button);
    ...
    g_signal_connect (send_button, "clicked", G_CALLBACK(send_clicked), NULL);
